...
if (!values.token) {
	console.error(
		"Usage: node --experimental-strip-types find-stale-issues.ts --token <github-token>",
	);
	process.exit(1);
}
...
const issues: { number: number; updatedAt: string }[] = JSON.parse(
	execSync(
		`gh issue list --label "S-Needs repro" --state open --json number,updatedAt --limit 500 --repo biomejs/biome-vscode`,
